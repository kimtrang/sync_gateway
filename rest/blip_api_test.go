...
func TestContinousChangesSubscription(t *testing.T) {
	...
	bt.blipContext.HandlerForProfile["changes"] = func(request *blip.Message) {
		...
		if string(body) != "null" {
			...
			log.Printf("changeListReceived: %v", changeListReceived)
			assert.True(t, len(changeListReceived) == 1)

			// Assuming that the callback receives a single change at a time, since
			// docs are being inserted one at a time
			change := changeListReceived[0] // [1,"foo","1-abc"]
			assert.True(t, len(change) == 3)
